...
export default function BurgerMenu({ open, onOpen, onClose }) {
  ...
  return (
    <>
      <button type="button" onClick={handleToggle} aria-label={open ? "Lukk meny" : "Åpne meny"} aria-expanded={open} aria-controls="burger-menu-panel" className="fixed top-4 right-4 z-[60] h-11 w-11 rounded-full bg-white/95 backdrop-blur-xl border border-[#EBE5DC] shadow-sm flex items-center justify-center text-[#2C2A26] no-tap-highlight touch-manipulation" data-testid="open-burger-menu">
        {open ? <X size={21} strokeWidth={1.6} /> : <Menu size={22} strokeWidth={1.6} />}
      </button>

      <div className={`fixed inset-0 z-50 transition-opacity duration-200 ${open ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"}`} data-testid="burger-menu" aria-hidden={!open}>
          ...
          <nav className="flex-1 overflow-y-auto px-4 py-5">
            <div className="space-y-1">
              {items.map(({ to, label, icon: Icon, end }) => (
                <NavLink key={to} to={to} end={end} tabIndex={open ? 0 : -1} onClick={onClose} className={({ isActive }) => `flex items-center gap-4 rounded-2xl px-4 py-3.5 transition-colors ${isActive ? "bg-[#F0E9DF] text-[#2C2A26]" : "text-[#5F594F] hover:bg-white"}`}>
